$(document).ready(function(){
	...
	function removeRowFromWinkelmandje(naam){
		$("#winkelmandje button#add"+naam).off(); //afzetten, omgekeerde van .on het klikevent wordt verwijderd
	...
	$("#pizzaLijst").change(function() {
		...
		if (selectedPizza == undefined){
			...
			$("#showPizzaName").fadeOut(400, function(){
					$("#showPizzaName").html(pizzaNaam)
			}).fadeIn(400);
			$("#showPizzaDetails").fadeOut(400,function(){ 
	...
	$("#orderPizza").click(function(){
		...
		var aantal = +$("#aantal").val(); //de plus ervoor zorgt er voor dat het gelezen wordt als een getal
	...
	$("#addIngredient").click(function(){
		...
		if (nieuwIngredient.length > 0){
			...
			$("<p>Geef ingredient in.</p>").hide()
			.insertAfter($("#ingredient"))
			.css("color","red")
			.fadeIn(1000)
			.addClass("errorAddIngredient");
			bestaatAl = true;
		}
	});
